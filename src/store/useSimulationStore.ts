  ...
  return {
    ...
    setParameters: (params: Partial<AllParameters>) => {
      ...
      if (oldEcosystemMode !== newEcosystemMode) {
        ...
        if (oldEngine) {
          logger.log('[Store] Ecosystem mode changed, replacing engine');
        }
    ...
    applyQualityPreset: (preset) => {
      ...
      logger.log(`[Quality Preset] Applied "${preset}" preset:`, settings);
    },
    ...
    performAutoOptimization: () => {
      ...
      if (fpsRatio < 0.5 && newOptLevel < 10) {
        ...
        logger.log(`[Auto-Optimizer] FPS critical (${avgFPS.toFixed(1)}/${targetFPS}), jumping opt level: ${_currentOptLevel} → ${newOptLevel}`);
      } else if (fpsRatio < 0.7 && newOptLevel < 10) {
        newOptLevel = Math.min(10, _currentOptLevel + 2);
        logger.log(`[Auto-Optimizer] FPS very low (${avgFPS.toFixed(1)}/${targetFPS}), increasing opt level: ${_currentOptLevel} → ${newOptLevel}`);
      } else if (fpsRatio < 0.9 && newOptLevel < 10) {
        newOptLevel = Math.min(10, _currentOptLevel + 1);
        logger.log(`[Auto-Optimizer] FPS low (${avgFPS.toFixed(1)}/${targetFPS}), increasing opt level: ${_currentOptLevel} → ${newOptLevel}`);
      } else if (fpsRatio > 1.2 && newOptLevel > 0) {
        newOptLevel = Math.max(0, _currentOptLevel - 1);
        logger.log(`[Auto-Optimizer] FPS good (${avgFPS.toFixed(1)}/${targetFPS}), decreasing opt level: ${_currentOptLevel} → ${newOptLevel}`);
      }
    ...
    setAspectRatio: (ratio) => {
      ...
      if (oldEngine) {
        logger.log(`[Store] Aspect ratio changed to ${ratio}, replacing engine (grid: ${width}x${height})`);
      }
    ...
    importPresetFromFile: async (file: File) => {
      logger.log('[Store] importPresetFromFile called with:', file.name);
      const { preset, error } = await importPresetFromJSON(file);

      logger.log('[Store] Import result:', { hasPreset: !!preset, hasError: !!error });

      if (error) {
        logger.error('[Store] Import failed with error:', error);
        return { success: false, error };
      }

      if (preset && preset.parameters) {
        logger.log('[Store] Loading preset parameters...');
        const { loadPreset } = get();
        loadPreset(preset.parameters);
        logger.log('[Store] Preset loaded successfully');
        return { success: true };
      }

      logger.error('[Store] Invalid preset format: preset or parameters missing');
