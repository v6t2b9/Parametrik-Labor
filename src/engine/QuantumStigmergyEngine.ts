...
export class QuantumStigmergyEngine {
  ...
  private senseM3(
    ...
    switch (agent.type) {
      case 'red':
        totalMagnitude = (effectiveStrength * matrix.redToRed * attractionMult) +
          (crossSpecies ? (trails.green[idx] * matrix.redToGreen + trails.blue[idx] * matrix.redToBlue) * repulsionMult : 0);
        break;
      case 'green':
        totalMagnitude = (effectiveStrength * matrix.greenToGreen * attractionMult) +
          (crossSpecies ? (trails.red[idx] * matrix.greenToRed + trails.blue[idx] * matrix.greenToBlue) * repulsionMult : 0);
        break;
      case 'blue':
        totalMagnitude = (effectiveStrength * matrix.blueToBlue * attractionMult) +
          (crossSpecies ? (trails.red[idx] * matrix.blueToRed + trails.green[idx] * matrix.blueToGreen) * repulsionMult : 0);
        break;
      default:
        totalMagnitude = 0;
